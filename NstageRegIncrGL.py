...
class NstageRegIncrGL( Component ):
  def construct( s, nbits=4, nstage=2 ):
    s.in_ = InPort (nbits)
    s.out = OutPort(nbits)

    s.rincrs = [ RegIncrGL(nbits) for _ in range(nstage) ]
...
if __name__ == "__main__":
  ...
  # Turn on Verilog translation and import and apply corresponding passes
